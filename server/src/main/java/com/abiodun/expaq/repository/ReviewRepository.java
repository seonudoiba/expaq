...
public interface ReviewRepository extends JpaRepository<Review, UUID> {
    ...
    Page<Review> findByActivityId(UUID activityId, Pageable pageable);
    ...
    @Query("SELECT r FROM Review r WHERE r.user.id = :userId AND r.activity.id = :activityId")
    Review findByUserIdAndActivityId(@Param("userId") UUID userId, @Param("activityId") UUID activityId);

    @Query("SELECT r FROM Review r WHERE r.user.id = :userId AND r.booking.id = :bookingId")
    ...
    @Query("SELECT r FROM Review r WHERE r.host.id = :hostId AND r.rating >= :minRating AND r.rating <= :maxRating")
    ...
    @Query("SELECT r FROM Review r WHERE r.activity.id = :activityId AND r.verified = :verified")
    Page<Review> findByActivityIdAndVerified(
            @Param("activityId") UUID activityId,
            @Param("verified") boolean verified,
            Pageable pageable);

    @Query("SELECT r FROM Review r WHERE r.host.id = :hostId AND r.verified = :verified")
    Page<Review> findByHostIdAndVerified(
            @Param("hostId") UUID hostId,
            @Param("verified") boolean verified,
            Pageable pageable);

    @Query("SELECT r FROM Review r WHERE r.activity.id = :activityId AND r.edited = :edited")
    Page<Review> findByActivityIdAndEdited(
            @Param("activityId") UUID activityId,
            @Param("edited") boolean edited,
            Pageable pageable);

    @Query("SELECT r FROM Review r WHERE r.host.id = :hostId AND r.edited = :edited")
    ...
    @Query("SELECT r FROM Review r WHERE r.host.id = :hostId AND r.createdAt BETWEEN :startDate AND :endDate")
    ...
    @Query("SELECT r FROM Review r WHERE r.host.id = :hostId AND r.rating IN :ratings")
    ...
    @Query("SELECT AVG(r.rating) FROM Review r WHERE r.host.id = :hostId")
    ...
    @Query("SELECT COUNT(r) FROM Review r WHERE r.host.id = :hostId")
    int countByHostId(@Param("hostId") UUID hostId);
    ...
    @Query("SELECT COUNT(r) FROM Review r WHERE r.host.id = :hostId AND r.rating = :rating")
    int countByHostIdAndRating(@Param("hostId") UUID hostId, @Param("rating") int rating);

    @Query("SELECT COUNT(r) FROM Review r WHERE r.activity.id = :activityId AND r.verified = :verified")
    int countByActivityIdAndVerified(@Param("activityId") UUID activityId, @Param("verified") boolean verified);

    @Query("SELECT COUNT(r) FROM Review r WHERE r.host.id = :hostId AND r.verified = :verified")
    int countByHostIdAndVerified(@Param("hostId") UUID hostId, @Param("verified") boolean verified);

    @Query("SELECT COUNT(r) FROM Review r WHERE r.activity.id = :activityId AND r.edited = :edited")
    int countByActivityIdAndEdited(@Param("activityId") UUID activityId, @Param("edited") boolean edited);

    @Query("SELECT COUNT(r) FROM Review r WHERE r.host.id = :hostId AND r.edited = :edited")
    int countByHostIdAndEdited(@Param("hostId") UUID hostId, @Param("edited") boolean edited);
    ...
    @Query("SELECT r FROM Review r WHERE r.host.id = :hostId ORDER BY r.createdAt DESC")
    Page<Review> findLatestByHostId(@Param("hostId") UUID hostId, Pageable pageable);

    @Query("SELECT r FROM Review r WHERE " +
            "(:activityId IS NULL OR r.activity.id = :activityId) AND " +
            "(:userId IS NULL OR r.user.id = :userId) AND " +
            "(:hostId IS NULL OR r.host.id = :hostId) AND " +
            "(:minRating IS NULL OR r.rating >= :minRating) AND " +
            "(:maxRating IS NULL OR r.rating <= :maxRating) AND " +
            "(:isVerified IS NULL OR r.verified = :isVerified) AND " +
            "(:isEdited IS NULL OR r.edited = :isEdited) AND " +
